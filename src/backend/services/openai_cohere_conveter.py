...
class CohereToOpenAI:
    ...
    def openai_to_cohere_event_chunk(event: ChatCompletionChunk, previous_response: Optional[str] = None, function_triggered: str = 'none', chat_request: CohereChatRequest = None, generation_id: Optional[str] = "", build_template: bool = False) -> list[StreamedChatResponse] | None:
        ...
        if (event.choices[0].finish_reason == "tool_calls" or event.choices[0].finish_reason == "function_call" or (event.choices[0].delta and event.choices[0].delta.function_call)):
    ...
    def _create_request_with_template(
        ...
        return RegularCompletionCreateParamsBase(
            ...
            model=cohere_request.model,  # type: ignore
            max_tokens=cohere_request.max_tokens or 10000,
            temperature=cohere_request.temperature,
            frequency_penalty=cohere_request.frequency_penalty,
            presence_penalty=cohere_request.presence_penalty,
            stop=cohere_request.stop_sequences,
